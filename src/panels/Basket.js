...
const Basket = ({
    ...
    const store = JSON.parse(localStorage.getItem(`${areaId}_${itemId}`) || null) || {}
    const [faster, setFaster] = useState(store.faster ?? true)
    const [time, setTime] = useState(store.time || '')
    const [selfService, setSelfService] = useState(store.service || false)
    const area = foodAreas.filter((area) => area.id === areaId)[0]
    const item = area.items.filter((item) => item.id === itemId)[0]

    const onClickPay = () => {
        const s = { faster: faster, time: time, service: selfService }
        localStorage.setItem(`${areaId}_${itemId}`, JSON.stringify(s))
    }
    ...
    return (
        <div className="Place">
            ...
            <aside className="Place__restoraunt">
                <img className="Place__restoraunt-logo" alt="Fastfood logo" src={item.image} />
            ...
            <div className="Place__products-wrapper">
                ...
                <Link className="Place__change-product" to={`/place/${areaId}/${itemId}`}>
                    Изменить
                </Link>
            </div>
            <div className="Place__choice">
                ...
                <div className="Place__choice-item">
                    ...
                    <input
                        ...
                        onChange={(event) => {
                            setFaster(false)
                            setTime((event.hour || '00') + ':' + (event.minute || '00'))
                        }}
                        onBlur={() => {
                            if (time) {
                                setFaster(false)
